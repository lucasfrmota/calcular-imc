...
df['Nome completo'] = df.apply(lambda x: x['Primeiro Nome'].strip() + ' ' +  x['Sobrenomes'], axis = 1)
df['Nome completo'] = df['Nome completo'].str.upper()

df_final = df[['Nome completo', 'IMC']]

separaSobrenome = df['Sobrenomes'].str.split()
print(separaSobrenome.str.capitalize())

#df_final.to_csv('[lucasFreireMota].txt', header = False, index=False, sep=" ")